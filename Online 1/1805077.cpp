...
void display()
{
    ...
    // draw
    glPushMatrix();
        glRotatef(objectOrbitAngle, upDir.x, upDir.y, upDir.z);
        // glRotatef(objectRotationHorAngle, 1, 0, 0);
        glPushMatrix();
            // glRotatef(objectRotationVerAngle, 1, 0, -1);
            // glRotatef(objectOrbitAngle, 0, 1, 0);
            if (isAxes)         drawAxes(2);
            // if (isOctahedron)   drawOctahedron();
            glPushMatrix();
            glTranslatef(10 * cos(objectOrbitAngle), 0, 10 * sin(objectOrbitAngle));
